  ...
  <div class="popup">
    ...
    <div class="mt-10">
      <div v-for="prop in schemaProps">
        <br />
        <Input
          v-if="prop.value.type == 'string'"
          type="text"
          :label="prop.key.toLocaleUpperCase()"
          :placeholder="prop.value.placeholder"
          name="prop.key"
          v-model="prop.value.data"
          class="ae-address"
        />
        <Input
          v-if="prop.value.type == 'date'"
          type="date"
          :label="prop.key"
          name="prop.key"
          :placeholder="prop.value.placeholder"
    ...
    async onSelect(event) {
      try {
        ...
        if (this.selected.value != null && this.selected.value.includes('sch:hid:testnet')) {
          ...
          this.schemaProps = Object.entries(JSON.parse(this.schema.properties)).map(
            ([key, value]) => {
              switch (key) {
                case 'photo':
