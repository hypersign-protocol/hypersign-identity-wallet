  ...
  <div class="popup">
    ...
    <div v-else style="max-height: 600px; overflow-y: scroll">
      <div
        ...
        :key="credential.id"
        @click="moveTo(`/credential/${credential.id}`)"
      >
        <div class="cred-card-header">
          <div style="float: left">
            <div class="whiteFont" style="font-size:large; font-weight: bold;">
              {{ shortner(credential.type[1], 19, 10) }}
            </div>
            <div class="fieldName">
              {{ schemaIdFormatted(credential['credentialSchema']['id']).toUpperCase() }}
            </div>
          </div>
        ...
        <div class="cred-card-body container">
          <div
            style="width: 100%; padding: 5px;   height: 3vh;margin-top: 14px;color: #CCCCCC; font-weight: bold;   font-size: 14px"
          >
    ...
    if (localStorage.getItem('3rdPartyAuthVCUnregistred')) {
      ...
      try {
        ...
        localStorage.setItem(
          '3rdPartyAuthVCUnregistred',
          JSON.stringify(this.verifiableCredential),
        );
